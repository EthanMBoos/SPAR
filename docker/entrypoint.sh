...
# compose.yaml), so `make dev` only builds the image and starts
...
# (localization and Nav2, see navigation.launch.py) is meant to be run
...
# The container just needs to stay up for `make dev`/`make shell` to exec
