...
def launch_stack(context):
    ...
    if not world:
        raise RuntimeError("world is required for ground datum, dock, and route config")
    ...
    def node(executable, package_name="spar_ground", extra_parameters=()):
        return Node(
            package=package_name,
            executable=executable,
            namespace=namespace,
            parameters=[
                *node_params, *extra_parameters, {"use_sim_time": True}],
    ...
    def localization_node(executable, name, remappings=()):
        return Node(
            package="robot_localization",
            executable=executable,
            name=name,
            namespace=namespace,
            parameters=[localization_params, world_params, {"use_sim_time": True}],
    ...
    return [
        nav2,
        node("wheel_odometry", extra_parameters=[localization_params]),
        localization_node(
            "ekf_node", "ekf_local",
            [("odometry/filtered", "platform/odom")]),
        localization_node(
            "ekf_node", "ekf_global",
            [("odometry/filtered", "localization/global")]),
        localization_node(
            "navsat_transform_node", "navsat_transform",
            [
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description="world name selecting ground datum, dock, and route config"),
