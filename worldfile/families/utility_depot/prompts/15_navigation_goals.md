...
`worldfile.guide_frame(exact_name)`. Plan guides are Blender Empties: never read
their `dimensions`, `bound_box`, `matrix_world`, or display size directly.
Look up `SITE_HuskySpawn` with `worldfile.site(...)`, and find the one inspection
target with `worldfile.roots_by_role("anomaly")`. Do not make a separate inspection
call.
...
spawn. Set its heading with `worldfile.inspection_yaw(target, point,
...
Compute ordinary headings with `worldfile.route_headings(points, face="arrival")`,
replace the first heading with the verified inspection yaw, and create the
ordered goal sites with
`worldfile.add_ordered_sites("SITE_NavigationGoal", "navigation_goal", points,
