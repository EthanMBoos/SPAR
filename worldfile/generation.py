...
from .georeference import family_georeference, valid_georeference
...
def derive_seed(seed: int, family: str, namespace: str) -> int:
    ...
    payload = f"worldfile-worldgen-v{SCHEMA_VERSION}\0{family}\0{seed}\0{namespace}"
    return int.from_bytes(hashlib.sha256(payload.encode()).digest()[:8], "big")


def family_spawn_defaults(family: str, seed: int) -> dict[str, Any]:
    """Resolve repo-owned spawn defaults before any LLM stage runs."""
    if family == "utility_depot":
        from .families.utility_depot.spawns import sample_spawn_defaults

        return sample_spawn_defaults(derive_seed(seed, family, "spawn_defaults"))
    return {}


def _valid_spawn_defaults(value: Any) -> bool:
    if not isinstance(value, dict):
        return False
    for role, pose in value.items():
        if not isinstance(role, str) or not isinstance(pose, dict):
            return False
...
def build_manifest(
    ...
    *,
    ...
    return {
        ...
        "model": model,
        "effort": effort,
        "spawn_defaults": family_spawn_defaults(family, seed),
        "georeference": family_georeference(family),
...
def load_manifest(world: str) -> dict[str, Any]:
    ...
    required = {
        ...
        "effort", "spawn_defaults", "georeference", "generator", "stages",
    }
    ...
    try:
        ...
        spawn_defaults = manifest["spawn_defaults"]
    ...
    if (
        ...
        or not _valid_spawn_defaults(spawn_defaults)
        or not valid_georeference(georeference)
        or not valid_stages
    ):
        raise ValueError(f"invalid generation recipe: {path}")
    return manifest
    ...
    *,
    ...
    if existing:
        ...
        if manifest["generator"]["source_sha256"] != current_source_hash:
            raise ValueError("Worldfile source changed since the recipe was created")
        return manifest
...
def environment_seed() -> str | None:
    return os.environ.get("WORLDFILE_WORLDGEN_SEED") or None


def environment_brief() -> str | None:
    value = os.environ.get("WORLDFILE_WORLDGEN_BRIEF")
