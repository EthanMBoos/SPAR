...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate, export, and validate one complete Worldfile world."
    )
...
def export_and_validate(world: str) -> None:
    ...
    print("worldfile: exporting Blender scene to MJCF", flush=True)
    subprocess.run(export_command, cwd=REPO, check=True)
    print("worldfile: validating exported MJCF and rollouts", flush=True)
    subprocess.run(
...
def main() -> int:
    ...
    if args.world != requested_world:
        print(f"worldfile: normalized world name to {args.world}", flush=True)
    ...
    print(
        f"worldfile: recipe {generation.manifest_path(args.world)} "
    ...
    try:
        ...
        for name in stages:
            ...
            if result:
                print(
                    f"Worldfile stopped at {name}; Blender remains open for inspection",
                    file=sys.stderr,
                )
                return result
        ...
        if partial or args.no_export:
            print("worldfile: authoring stages complete; export skipped", flush=True)
            return 0
        export_and_validate(args.world)
    except (OSError, RuntimeError, subprocess.CalledProcessError) as error:
        raise SystemExit(f"Worldfile failed: {error}") from error

    print(f"Worldfile complete: {args.world}", flush=True)
    return 0
