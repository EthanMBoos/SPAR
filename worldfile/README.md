...
Worldfile turns one environment family, seed, and short brief into a reviewed
...
```bash
uv run python -m worldfile utility_depot_40_v3 \
...
uv run python -m worldfile.stage utility_depot_40_v3 05_build_structures
uv run python -m worldfile utility_depot_40_v3 --dry-run
uv run python -m worldfile utility_depot_40_v3 --start-at 11_detail_infrastructure
```
...
open -n -W -a Blender --args --background \
  ...
  --python-exit-code 1 --python "$PWD/worldfile/export.py" -- \
  --world "$WORLD" --navigation-goals

uv run python worldfile/check_export.py "$WORLD"
```

Export produces:

- `sim/worlds/<world>.xml` and `sim/worlds/assets/<world>/`;
- `ros/src/worldfile_demo/config/worlds/<world>.yaml` with `navsat_datum`, the
  authored dock pose, and ordered `{name, x, y, yaw}` navigation goals;
