"""Shared Blender mechanics for visible Worldfile world generation.
...
environment-family logic belongs in ``worldfile/families/<family>/helpers.py``
...
def dimensions_from_anchor(
    ...
    raw = anchor.get("worldfile_plan_dimensions")
    if raw is None:
...
def guide(name: str) -> bpy.types.Object:
    ...
    if obj is None or not obj.get("worldfile_plan_kind"):
...
def site(name: str) -> bpy.types.Object:
    """Return one named Worldfile site."""
    obj = bpy.data.objects.get(name)
    if obj is None or not obj.get("worldfile_site"):
        raise ValueError(f"missing Worldfile site: {name}")
    return obj


def plan_guides(kind: str | None = None) -> list[bpy.types.Object]:
    guides = [obj for obj in bpy.data.objects if obj.get("worldfile_plan_kind")]
    if kind is not None:
        guides = [obj for obj in guides if obj.get("worldfile_plan_kind") == kind]
...
def assembly_anchors(*plan_types: str) -> list[bpy.types.Object]:
    ...
    if wanted:
        anchors = [obj for obj in anchors if obj.get("worldfile_plan_type") in wanted]
    return anchors


def roots_by_type(*worldfile_types: str) -> list[bpy.types.Object]:
    wanted = set(worldfile_types)
    roots = [obj for obj in bpy.data.objects if obj.get("worldfile_type")]
    if wanted:
        roots = [obj for obj in roots if obj.get("worldfile_type") in wanted]
    return sorted(roots, key=lambda obj: obj.name)


def roots_by_role(role: str) -> list[bpy.types.Object]:
    return sorted(
        [obj for obj in bpy.data.objects if obj.get("worldfile_role") == role],
...
def add_plan_guide(
    ...
    *,
    ...
    obj["worldfile_plan_kind"] = kind
    obj["worldfile_plan_dimensions"] = list(dims)
    return obj
...
def add_plan_anchor(
    ...
    *,
    ...
    obj["worldfile_plan_kind"] = "assembly"
    obj["worldfile_plan_type"] = plan_type
    obj["worldfile_plan_zone"] = zone
    obj["worldfile_plan_dimensions"] = list(dims)
    obj["worldfile_plan_origin"] = "base"
    if role:
        obj["worldfile_plan_role"] = role
    return obj
...
def add_site(
    ...
    *,
    ...
    obj = add_empty(
        ...
        collection="WORLDFILE_SITES",
    )
    obj["worldfile_site"] = site
    if order is not None:
        obj["worldfile_order"] = int(order)
    return obj
...
def semantic_root(
    ...
    *,
    worldfile_type: str | None = None,
    collection: str = "PROPS",
) -> bpy.types.Object:
    """Create an assembly root whose world transform exactly matches an anchor."""
    anchor_type = str(anchor.get("worldfile_plan_type", "assembly"))
    if worldfile_type is not None and worldfile_type != anchor_type:
        raise ValueError(
            f"semantic root type must preserve anchor type {anchor_type!r}, "
            f"got {worldfile_type!r}"
        )
    ...
    root["worldfile_type"] = anchor_type
    root["worldfile_plan_anchor"] = anchor.name
    role = anchor.get("worldfile_plan_role")
    if role:
        root["worldfile_role"] = role
    return root
...
def _finish_primitive(
    ...
    if no_collision:
        obj["worldfile_no_collision"] = True
    return obj
...
def add_box_local(
    ...
    *,
    ...
    if bevel > 0.0:
        modifier = obj.modifiers.new("Worldfile_Bevel", "BEVEL")
...
def write_scene_manifest(path: str) -> None:
    ...
    payload = {
        ...
        "objects": [
            {
                "name": obj.name,
                "worldfile_type": obj.get("worldfile_type"),
