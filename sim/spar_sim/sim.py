...
def resolve_ids(model):
    ...
    if missing:
        sys.exit(f"[spar] sim could not resolve ids: missing {missing}, "
                 f"resolved {ids}")
    print(f"[spar] sim ids ok: body={ids['base_link']} site={ids['lidar']} "
          f"actL={ids['act_left']} actR={ids['act_right']}", flush=True)
    return ids
...
def main():
    ...
    while rclpy.ok():
        ...
        if stamp < 0.0 or data.time - stamp > CMD_TIMEOUT_SEC:
            v = w = 0.0
        data.ctrl[ids["act_left"]] = (v - w * TRACK_WIDTH / 2.0) / WHEEL_RADIUS
        data.ctrl[ids["act_right"]] = (v + w * TRACK_WIDTH / 2.0) / WHEEL_RADIUS
        ...
        if t >= next_odom:
            next_odom = t + 1.0 / ODOM_RATE
            clock_pub.publish(Clock(clock=sensors.stamp_of(t)))
            odom, tf = sensors.odom_and_tf(model, data, ids["base_link"], t)
