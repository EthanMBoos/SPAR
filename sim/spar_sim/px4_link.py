...
# Geodetic origin (EKF2's local origin lands here): GT campus.
ORIGIN_LAT_DEG = 33.7756
ORIGIN_LON_DEG = -84.3963
ORIGIN_ALT_M = 300.0
...
class Px4Link:
    ...
    def __init__(self, model, port=LISTEN_PORT):
    ...
    def _send_hil_sensor(self, model, data, t):
        ...
        alt = data.xpos[self._body][2] + ORIGIN_ALT_M
    ...
    def _send_hil_gps(self, model, data, t):
        ...
        mujoco.mj_objectVelocity(model, data, mujoco.mjtObj.mjOBJ_XBODY,
                                 self._body, vel, 0)
        vn, ve, vd = vel[4], vel[3], -vel[5]

        earth_r = 6378137.0
        lat_rad = math.radians(ORIGIN_LAT_DEG)
        lat = ORIGIN_LAT_DEG + math.degrees(north / earth_r)
        lon = ORIGIN_LON_DEG + math.degrees(east / (earth_r * math.cos(lat_rad)))
        ...
        payload = struct.pack(
            ...
            int(lat * 1e7),
            int(lon * 1e7),
            int((ORIGIN_ALT_M + up) * 1000.0),
            30, 60,  # eph/epv, dilution * 100
