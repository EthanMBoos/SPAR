...
Run it headless in the sim container:
    ...
    MUJOCO_GL=egl WORLD=utility_depot_40_v2 python3 -m worldfile_sim.sim
...
from worldfile_sim import sensors
from worldfile_sim.georeference import from_mjcf
from worldfile_sim.husky import (
    KinematicDrive,
    WHEEL_NAMES,
)
from worldfile_sim.robot_config import scan_site
...
def resolve_ids(model):
    ...
    try:
        ...
        sys.exit(f"[worldfile] sim could not resolve ids: {exc}")
    ids = {
    ...
    if missing:
        sys.exit(f"[worldfile] sim could not resolve ids: missing {missing}, "
                 f"resolved {ids}")
    print(
        f"[worldfile] sim ids ok: body={ids['base_link']} site={ids['lidar']} "
...
def main():
    ...
    rclpy.init()
    node = rclpy.create_node("worldfile_sim", namespace="husky")
    ...
    print("[worldfile] Husky RGB-D camera mounted", flush=True)
    lidar = sensors.Lidar()
