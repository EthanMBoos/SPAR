...
public:
  ...
  void init() {
    ...
    link_ = std::make_unique<OffboardLink>(*this, *transform_);
    ...
    pad_params.home_x = get_parameter("home_x").as_double();
    pad_params.home_y = get_parameter("home_y").as_double();
    ...
    Land::Params land_params;
    land_params.home_z = get_parameter("home_z").as_double();
        ...
        [this](const px4_msgs::msg::VehicleLocalPosition& msg) {
          ...
          if (update.xy_reset || update.z_reset) {
            RCLCPP_WARN(get_logger(),
                        "PX4 position estimate reset (xy=%s, z=%s); "
                        "map target remains earth-fixed",
    ...
    odometry_sub_ = create_subscription<px4_msgs::msg::VehicleOdometry>(
        ...
        [this, blackboard](const px4_msgs::msg::VehicleOdometry& msg) {
          if (msg.pose_frame !=
          ...
          blackboard->set<Stamped<Vec3>>(
              keys::kPosition,
              {*map, static_cast<double>(msg.timestamp_sample) * 1e-6});
        });
