...
class OffboardLink {
public:
  OffboardLink(rclcpp::Node& node, const Px4MapTransform& transform)
      : node_(node), transform_(transform) {
  ...
  Vec3 target() const { return target_.value_or(Vec3{}); }
...
private:
  void stream() {
    ...
    if (!target_) return;
    const auto ned = transform_.mapToLocalNed(*target_);
    if (!ned) return;
    ...
    sp.jerk = {nan, nan, nan};
    sp.yaw = static_cast<float>(yawEnuToNed(yaw_));
    sp.yawspeed = nan;
  ...
  rclcpp::Node& node_;
  const Px4MapTransform& transform_;
