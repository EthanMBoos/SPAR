...
def add_sites(worldbody):
    ...
    for obj in sorted(sites_collection.objects, key=lambda item: item.name):
        ...
        ET.SubElement(
            ...
            pos=vector_text(location), **{"class": "spar_world_site"},
        )
        exported.append({
            "name": obj.name,
            "role": role,
...
def ground_route_in_local_frame(sites):
    spawns = [site for site in sites if site["role"] == "husky_spawn"]
    if len(spawns) != 1:
        raise RuntimeError("the ground route needs exactly one husky_spawn site")
    spawn = spawns[0]
...
def write_ground_waypoint_config(world, sites, colliders, enabled):
    ...
    spawn, route = ground_route_in_local_frame(sites)
...
def validate_air_route(spawn, route, colliders, site_dimensions):
    ...
    if any(collider_overlaps_pad(
            collider, spawn["position"][0], spawn["position"][1])
...
def write_air_waypoint_config(
        world, sites, colliders, site_dimensions, enabled):
