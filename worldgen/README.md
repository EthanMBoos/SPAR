...
repo-owned choices; it cannot make Claude output identical. `BRIEF` is an
optional supported instance override. The resolved recipe, rendered prompts,
and raw stage traces are saved under `artifacts/worldgen/<world>/`.
...
| Stage | Inspect |
| --- | --- |
| `01_plan_topology` | Boundary, gate, aisles, separated zones |
...
| `15_ground_waypoints` | Traversable route; early target LOS, range, and yaw |
