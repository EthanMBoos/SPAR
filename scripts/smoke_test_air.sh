...
local_position_ready() {
  local message
  message="$(timeout 10 ros2 topic echo --once --qos-profile sensor_data \
    "$NS/fmu/out/vehicle_local_position" 2>/dev/null)" || return 1
  grep -q '^xy_valid: true' <<<"$message" &&
    grep -q '^z_valid: true' <<<"$message" &&
    grep -q '^xy_global: true' <<<"$message" &&
    grep -q '^z_global: true' <<<"$message"
}

estimated_odometry_ready() {
  timeout 10 ros2 topic echo --once --qos-profile sensor_data \
    "$NS/fmu/out/vehicle_odometry" 2>/dev/null \
    | grep -q '^pose_frame: 1'
}

map_tf_ready() {
  timeout 10 ros2 topic echo --once "$NS/tf" tf2_msgs/msg/TFMessage 2>/dev/null \
    | grep -q 'child_frame_id: base_link'
}
...
wait_for "valid GPS-backed PX4 local/global reference" 240 \
  "local_position_ready" || exit $FAIL
wait_for "synchronized NED VehicleOdometry" 60 \
  "estimated_odometry_ready" || exit $FAIL
wait_for "world-aligned map -> base_link TF" 60 \
  "map_tf_ready" || exit $FAIL
